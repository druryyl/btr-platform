 ...
 *
 * ```text
 * Empty -- customer selected --> Header Ready -- first item line --> Items Present
 *   -- all lines valid --> Ready To Save -- Save (local write, no network) --> Saved -> back
 * ```
 *
 * Capture is offline-first (BG-003): item identification reuses
 * [BarcodeScannerView] (scan) and the existing local Item Search (BR-009); every
 * reference resolves against the local Room caches and no capture step issues a
 * network call (P-07, §17.1). Warehouse is fixed to the session binding
 * (BR-005/006) and never editable.
 *
 * `Save` is enabled only when a Customer is set and at least one valid item
 * line exists (BR-001/002, BR-012, IR-M1/M2); it writes a local `DRAFT` only
 * (§14.1). Destination wiring (routes) is owned by S4.11.
 */
...
fun CreateReturnOrderScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            if (saved) {
                // Saved (§14.1) ──▶ back (local write only, no network).
            ...
            // Header Region: Warehouse is fixed to the session binding and
            // never editable (BR-005/006).
            OutlinedTextField(
                value = warehouseCode,
                onValueChange = {},
                enabled = false,
            ...
            // Added lines (add/remove lines, §12.2).
            ...
            // Action Region (Save, Cancel, §12.2). Save is enabled only when a
            // Customer is set and at least one valid line exists (§14.1).
            Button(
...
/** One added item line with its remove action (§12.2). */
@Composable
