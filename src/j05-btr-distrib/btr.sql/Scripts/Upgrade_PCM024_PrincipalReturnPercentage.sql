-- PCM-024 Principal Return Percentage snapshot.
-- Stores PRN-RET-004 from stored PRN-RET-003 and PRN-SALES-001.
-- Does not write or update PRN-SALES-001 or PRN-RET-003.
SET NOCOUNT ON;
GO

IF OBJECT_ID(N'dbo.BTRPD_PrincipalReturnPercentageKpi', N'U') IS NULL
BEGIN
...
IF OBJECT_ID(N'dbo.BTRPD_PrincipalReturnPercentage', N'U') IS NULL
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = N'IX_BTRPD_PrincipalReturnPercentage_SnapshotKey_SortOrder'
      AND object_id = OBJECT_ID(N'dbo.BTRPD_PrincipalReturnPercentage'))
BEGIN
