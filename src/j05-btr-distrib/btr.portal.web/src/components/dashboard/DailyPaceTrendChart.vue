...
const hasData = computed(
  () => props.dailyPace?.some((day) => day.IsElapsed && day.ActualAmount > 0) ?? false,
)

const projectedDaily = computed(
  () => props.dailyPace.find((day) => day.ProjectedDailyAmount > 0)?.ProjectedDailyAmount ?? 0,
)
  ...
  datasets: [
    {
      type: 'bar' as const,
      label: 'Daily actual',
      data: props.dailyPace.map((day) => (day.IsElapsed ? day.ActualAmount : 0)),
  ...
  <Card class="daily-pace-trend-chart">
    <template #title>
      <div class="daily-pace-trend-chart__title">
        ...
        <span>Daily Pace Trend</span>
      </div>
    </template>

    <template #content>
      <div v-if="loading" class="daily-pace-trend-chart__loading">
      ...
      <template v-else>
        ...
        <p v-else class="daily-pace-trend-chart__empty">
          No daily pace data for the current period.
        </p>
      </template>
    </template>
  </Card>
...
<style scoped>
.daily-pace-trend-chart__title {
  display: flex;
  align-items: center;
  gap: 0.5rem;
}

.daily-pace-trend-chart__loading {
  display: flex;
