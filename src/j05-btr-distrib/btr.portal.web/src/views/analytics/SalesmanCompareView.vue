...
function applyEntitiesFromQuery() {
  ...
  slots.value = codes.slice(0, MAX_ENTITIES).map((code) => ({
    ...
    DisplayName: code,
    IsActive: true,
    ProfileRoute: `/analytics/salesmen/${code}`,
  }))
...
<template>
  <DashboardDetailLayout
    ...
    @refresh="runCompare()"
  >
    <section class="salesman-compare-view__pickers">
      <div
        v-for="(_slot, index) in slots"
        :key="index"
        class="salesman-compare-view__picker-row"
      >
      ...
      <div class="salesman-compare-view__actions">
        <Button
    ...
    <Message
      ...
      severity="warn"
      :closable="false"
      class="salesman-compare-view__warning"
    >
      Compared entities were refreshed at different times. Snapshot timestamps may not align.
    </Message>

    <div v-if="store.compare" class="salesman-compare-view__sections">
...
<style scoped>
.salesman-compare-view__pickers {
  display: flex;
...
.salesman-compare-view__picker-row {
  display: grid;
...
.salesman-compare-view__picker-row label {
  font-size: 0.875rem;
  color: var(--p-text-muted-color, #64748b);
}

.salesman-compare-view__actions {
  display: flex;
  gap: 0.75rem;
  margin-top: 0.5rem;
}

.salesman-compare-view__warning {
  margin-bottom: 1rem;
}

.salesman-compare-view__sections {
  display: flex;
  flex-direction: column;
  gap: 0;
}
