...
function applyEntitiesFromQuery() {
  ...
  slots.value = codes.slice(0, MAX_ENTITIES).map((code) => ({
    ...
    DisplayName: code,
    IsActive: true,
    ProfileRoute: `/analytics/suppliers/${code}`,
  }))
...
<template>
  <DashboardDetailLayout
    ...
    @refresh="runCompare()"
  >
    <section class="supplier-compare-view__pickers">
      <div
        v-for="(_slot, index) in slots"
        :key="index"
        class="supplier-compare-view__picker-row"
      >
      ...
      <div class="supplier-compare-view__actions">
        <Button
    ...
    <Message
      ...
      severity="warn"
      :closable="false"
      class="supplier-compare-view__warning"
    >
      Compared entities were refreshed at different times. Snapshot timestamps may not align.
    </Message>

    <div v-if="store.compare" class="supplier-compare-view__sections">
...
<style scoped>
.supplier-compare-view__pickers {
  display: flex;
...
.supplier-compare-view__picker-row {
  display: grid;
...
.supplier-compare-view__picker-row label {
  font-size: 0.875rem;
  color: var(--p-text-muted-color, #64748b);
}

.supplier-compare-view__actions {
  display: flex;
  gap: 0.75rem;
  margin-top: 0.5rem;
}

.supplier-compare-view__warning {
  margin-bottom: 1rem;
}

.supplier-compare-view__sections {
  display: flex;
  flex-direction: column;
  gap: 0;
}
