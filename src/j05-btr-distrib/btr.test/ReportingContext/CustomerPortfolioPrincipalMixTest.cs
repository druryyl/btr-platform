...
using btr.application.ReportingContext.DashboardCustomerPortfolioAgg;
...
{
    public class CustomerPortfolioPrincipalMixTest
    {
        [Fact]
        public void Compose_ListsProjectionPairs_WithStoredStatusAndPairSalesOut()
        {
            var portfolio = Portfolio(Priority("C0001", "CLANDYS JAMAL", 100m));
            ...
            var mix = CustomerPortfolioPrincipalMixComposer.Compose(portfolio, projection);

            mix.IsAvailable.Should().BeTrue();
            mix.KpiId.Should().Be(PrincipalKpiCatalog.SalesOutId);
            mix.Customers.Select(row => row.CustomerCode).Should().Equal("C0001");

            var customer = mix.Customers.Single();
            ...
            portfolio.PriorityQueue[0].MtdOmzet.Should().Be(100m);
            mix.Disclosures.Should().Contain(CustomerPortfolioPrincipalMixComposer.CustomerLevelNote);
            mix.Disclosures.Should().Contain(CustomerPortfolioPrincipalMixComposer.ProjectionOnlyPairsNote);
        }

        [Fact]
        public void Compose_KeepsDifferentPairStatuses_ForOneCustomerAtTheSameTime()
        {
            var portfolio = Portfolio(Priority("CT", "INTI MART", 50m));
            var projection = Projection(
            ...
            var statuses = mix.Customers.Single().Principals
        ...
        [Fact]
        public void Compose_IsUnavailable_WhenProjectionIsMissing_AndLeavesPortfolioTotalsUnchanged()
        {
            var portfolio = Portfolio(Priority("C0001", "CLANDYS JAMAL", 100m));

            var mix = CustomerPortfolioPrincipalMixComposer.Compose(portfolio, projection: null);
        ...
        [Fact]
        public async Task Handle_AttachesProjectionMix_WithoutChangingPortfolioTotals()
        {
            var dal = new StubPortfolioDal(Portfolio(Priority("C0001", "CLANDYS JAMAL", 100m)));
            ...
            var handler = new GetDashboardCustomerPortfolioHandler(dal, relationshipDal);

            var result = await handler.Handle(new GetDashboardCustomerPortfolioQuery(), CancellationToken.None);

            result.PriorityQueue[0].MtdOmzet.Should().Be(100m);
            ...
            result.PrincipalMix.Customers.Single().Principals.Single().RelationshipStatus.Should()
                .Be(CustomerPrincipalRelationship.StatusDormant);
            relationshipDal.RequestedCustomerCodes.Should().Equal("C0001");
        }

        private static DashboardCustomerPortfolioResponse Portfolio(
            params DashboardCustomerPortfolioPriorityDto[] priority)
        {
            return new DashboardCustomerPortfolioResponse
            {
                IsAvailable = true,
                PriorityQueue = priority.ToList()
            };
        }

        private static DashboardCustomerPortfolioPriorityDto Priority(
            string customerCode,
            string customerName,
            decimal mtdOmzet)
        {
            return new DashboardCustomerPortfolioPriorityDto
            {
        ...
        private sealed class StubPortfolioDal : IDashboardCustomerPortfolioDal
        {
            private readonly DashboardCustomerPortfolioResponse _response;

            public StubPortfolioDal(DashboardCustomerPortfolioResponse response)
            {
                _response = response;
            }

            public DashboardCustomerPortfolioResponse GetCurrent() => _response;

            public DashboardCustomerPortfolioKpiDto GetCurrentKpi() => _response.Kpi;
        }
