...
WHERE bb.VoidDate = '3000-01-01'
  ...
  AND (aa.Total - ISNULL(pl.PaidThroughAsOf, 0)) > 1";
        ...
        public IReadOnlyList<PiutangOpenBalanceWithSalesmanDto> ListOpenBalancesAsOf(DateTime asOfDate)
        {
            using (var conn = new SqlConnection(ConnStringHelper.Get(_opt)))
            {
                return conn.Query<PiutangOpenBalanceWithSalesmanDto>(SqlAsOf, new { AsOfDate = asOfDate.Date }).ToList();
            }
        }
    }
}
