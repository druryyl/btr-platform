...
{
    public interface IRefreshPrincipalTargetSnapshotWorker
        : INunaServiceVoid<RefreshPrincipalTargetSnapshotRequest>
    {
    }

    public class RefreshPrincipalTargetSnapshotWorker : IRefreshPrincipalTargetSnapshotWorker
    {
        private const int MaxErrorMessageLength = 500;

        private readonly IPrincipalTargetEvidenceDal _evidenceDal;
        private readonly PrincipalTargetAggregator _aggregator;
        private readonly IPrincipalTargetSnapshotDal _snapshotDal;
        private readonly IDashboardSnapshotRefreshLogDal _refreshLogDal;
        private readonly ITglJamDal _tglJamDal;
        private readonly IBusinessDateProvider _businessDateProvider;

        public RefreshPrincipalTargetSnapshotWorker(
            IPrincipalTargetEvidenceDal evidenceDal,
            PrincipalTargetAggregator aggregator,
            IPrincipalTargetSnapshotDal snapshotDal,
        ...
        public void Execute(RefreshPrincipalTargetSnapshotRequest request)
        {
            ...
            var domain = PrincipalTargetSnapshot.Domain;
            ...
            try
            {
                ...
                WorkerProgressScope.Current.StepStarted($"{domain}:Load", "Load Salesman Principal Target evidence");
                var targets = _evidenceDal.ListSalesmanPrincipalTargets(periode.Tgl1.Year, periode.Tgl1.Month);
                WorkerProgressScope.Current.StepCompleted($"{domain}:Load", new WorkerProgressStepInfo
                {
                    RecordCount = targets?.Count ?? 0
                });

                WorkerProgressScope.Current.StepStarted($"{domain}:Aggregate", "Aggregate Principal Target");
                ...
                WorkerProgressScope.Current.StepStarted($"{domain}:Save", "Save Principal Target snapshot");
