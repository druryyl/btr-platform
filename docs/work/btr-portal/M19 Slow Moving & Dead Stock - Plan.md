...
- **Dedicated Inventory Risk snapshot domain** (`BTR_PortalDashboardInventoryRisk*`) with its own refresh worker — materialized KPIs, not live composition.
...
- Changes to existing Inventory snapshot tables (`BTR_PortalDashboardInventory*`) or `GET /api/dashboard/inventory`.
...
| Decision | Value |
| --- | --- |
| Snapshot domain | **`BTR_PortalDashboardInventoryRisk*`** (Q25) |
...
BTR_PortalDashboardInventoryRisk* tables (6)
    ↓
...
Existing unchanged in Phase 1:
  ...
  BTR_PortalDashboardInventory* worker and tables
...
| --- | --- | --- |
| SQL | `btr.sql/Tables/ReportingContext/BTR_PortalDashboardInventoryRisk*.sql` (6 tables) | **New** |
...
| Module | Reason |
| --- | --- |
| `DashboardInventoryAggregator`, `BTR_PortalDashboardInventory*` | M19 is additive; composition dashboard unchanged |
...
### 5.1 `BTR_PortalDashboardInventoryRiskKpi`
...
### 5.2 `BTR_PortalDashboardInventoryRiskAging`
...
### 5.3 `BTR_PortalDashboardInventoryRiskAttention`
...
### 5.4 `BTR_PortalDashboardInventoryRiskTopDead`
...
1. Create six `BTR_PortalDashboardInventoryRisk*.sql` table scripts.
...
3. Dedicated `BTR_PortalDashboardInventoryRisk*` tables exist and are populated by `RefreshDashboardInventoryRiskSnapshotWorker`.
