...
| PSOM-07 | 2 | Confidence guards and status | 2/5 | GO | 2026-09-13: Added shared `KpiConfidenceStatus` (`Normal`/`LowConfidence`) and trailing additive `ConfidenceStatus` on `PrincipalPacingAchievementRow`/`PrincipalYoyMtdGrowthRow`; added `PrincipalConfidenceGuard` (Services) reading `MinimumElapsedDays` for `PRN-TGT-004` and `MinimumBaseValue` for `PRN-GRW-003` from KPI Registry metadata (PSOM-03) via `IKpiRegistry.TryGetMetadata`, and suppressing the ratio value (`PacingAchievementPercentage`/`YoyMtdGrowthPercentage` → `null`) with `LowConfidence` on threshold breach, `Normal` otherwise; no threshold literals in code (pure static predicates + metadata resolvers). Registered `PrincipalConfidenceGuard` in `ApplicationPortalExtensions` DI + 2 source entries in `btr.application.csproj`; added focused `PrincipalConfidenceGuardTest` (11 tests, registered in `btr.test.csproj`) — VS MSBuild 18 (Release) green; focused 11/11 + related composer/period/range/catalog/registry suites 60/61 (1 pre-existing Customer-pack baseline failure, unrelated). Existing `PRN-TGT-003`/`PRN-GRW-002` and composer code untouched. | 2026-09-13: GO. AC1–AC4 PASS; FEASIBILITY authority honored (GAP-007/OQ-005: thresholds read from PSOM-03 KPI Registry metadata `MinimumElapsedDays`/`MinimumBaseValue`; no threshold literals in calculators; additive confidence status; existing PRN-TGT-003/PRN-GRW-002 untouched; no persistence/schema). Scope = 2 new source files (`KpiConfidenceStatus`, `PrincipalConfidenceGuard`) + additive `ConfidenceStatus` on the 2 result rows + `ApplicationPortalExtensions` DI + 1 test file (11 tests) + 2 csproj registrations + tracker; no expansion. Build evidence: VS MSBuild 18 (Release) green; focused `PrincipalConfidenceGuardTest` 11/11 green. INFO-001: threshold values (MinimumElapsedDays=6, MinimumBaseValue=1,000,000) live in PSOM-03 registrar metadata, not in guard/composer code. | — |
| PSOM-08 | 3 | Expose new KPIs to L0 via producer | 4/5 | IMPLEMENTED | 2026-09-13: Wired the PSOM-05/06/07 time-aware calculators into `SupplierEntityAnalyticsProducer` via three optional deps (`PrincipalPacingAchievementComposer`, `PrincipalYoyMtdGrowthComposer`, `PrincipalConfidenceGuard`; DI resolves them). `Produce` derives the PSOM-01 period context from the refresh Business Date (`AnalyticsPeriodCalculator.Create(context.BusinessDate, context.GeneratedAt)`), composes PRN-TGT-004 from the stored target + Sales-Out snapshots and PRN-GRW-003 from the PSOM-04 range evidence, applies the metadata-driven confidence guards (breached values suppressed to null), and maps both to L0 through the existing row patterns (per-portfolio compose + snapshot-only append + persisted retain) in both the normal and empty-portfolio paths. Existing KPIs untouched (new rows use the existing `AddKpiRow`); no new tables/columns/schema/migration; refresh cadence unchanged. Added 3 focused tests to `SupplierEntityAnalyticsProducerTest` (L0 exposure + guard-normal values, confidence suppression, existing-KPI preservation/uniqueness); no csproj change (same file). VS MSBuild 18 (Release) green; focused `SupplierEntityAnalyticsProducerTest` 8/9 (1 pre-existing baseline failure: L4 relationship row) and related Principal/registry suites 81/82 (pre-existing Customer exact-pack baseline failure). | 2026-09-13: GO. AC1–AC5 PASS; FEASIBILITY authority honored (GAP-001/GAP-003/GAP-004 dynamic computation, GAP-007 metadata-driven confidence suppression, GAP-008 registry registration consumed; Option B additive; existing PRN-SALES-001/PRN-TGT-003/PRN-GRW-002 and return KPIs untouched; no persistence/schema; no SA04). Scope = `SupplierEntityAnalyticsProducer.cs` (3 optional deps + Produce wiring + private composition/row helpers) + `SupplierEntityAnalyticsProducerTest.cs` (3 focused tests + helper/fakes) + tracker; no expansion. Build evidence: VS MSBuild 18 (Release) green; focused `SupplierEntityAnalyticsProducerTest` 8/9 (1 pre-existing baseline L4 relationship-row failure) and related Principal/registry suites 81/82 (pre-existing Customer exact-pack baseline). INFO-001: period context derives from the refresh Business Date via the PSOM-01 static `AnalyticsPeriodCalculator.Create` (worker supplies `_businessDateProvider.Today`), so no calculator dependency was injected; matches the PSOM-01 deterministic path. INFO-002: the target/sales-out snapshot DALS are read once more for the time-aware composition (also read by the existing target/sales-out compositions) — a minor additional read with no behavior change. | N/A |
| PSOM-09 | 3 | Preset repoint and lens membership | 1/5 | GO | 2026-09-13: Repointed `EntityMapPresetRegistry` `principal-sales-out-map` axes to `AxisXKpiId = "PRN-TGT-004"` / `AxisYKpiId = "PRN-GRW-003"` (`BubbleKpiId = "PRN-SALES-001"` / `BubbleColorKpiId = "PRN-RET-004"` unchanged). Added `PrincipalKpiCatalog.PacingAchievementPercentageId` (`PRN-TGT-004`) and `PrincipalKpiCatalog.YoyMtdGrowthId` (`PRN-GRW-003`) to the Sales-Out lens `KpiIds` in `EntityInvestigationLensRegistry` (registered identifiers only; no independent names/formulas). Updated `EntityMapPresetRegistryTest` axis assertions and `EntityInvestigationLensRegistryTest` expected KPI set. Existing `PRN-TGT-003`/`PRN-GRW-002` entries untouched and still available (Option B). Config-only; no persistence/schema/SA04 change. VS MSBuild 18 (Release) green; focused preset/lens suites 14/14 green. | 2026-09-13: GO. AC1–AC4 PASS; FEASIBILITY authority honored (GAP-009 registered identifiers only; Migration Option B — existing `PRN-TGT-003`/`PRN-GRW-002` retained and still available for SA04/ranking; no SA04 change). Scope = `EntityMapPresetRegistry.cs` (axis repoint) + `EntityInvestigationLensRegistry.cs` (2 lens KPI ids) + 2 test files + tracker; no expansion, no missing scope, no persistence/schema. Build evidence: VS MSBuild 18 (Release) green; focused preset/lens suites 14/14 green. INFO-001: PSOM-08 tracker Status cell reads IMPLEMENTED although its Review History records GO; PSOM-09 dependency treated as satisfied by the GO review record (not modified — outside target slice). | — |
| PSOM-10 | 3 | Population API confidence field | 3/5 | IMPLEMENTED | 2026-09-13: Added trailing additive `IsLowConfidence` on `PopulationMapPointDto` and populated it in `EntityPopulationMapEngine.BuildPopulationMap` (`true` when either resolved preset axis KPI value is absent/suppressed), and added optional `IsLowConfidence` to the `PopulationMapPoint` TypeScript model. Read-time projection only: no repository/schema/producer change; existing DTO fields unchanged. Added 2 focused tests to `EntityPopulationMapEngineTest` (flagged when an axis value is suppressed, unflagged when both present); VS MSBuild 18 (Release) green; focused `EntityPopulationMapEngineTest` 8/8 green. | 2026-09-13: GO. AC1–AC4 PASS; FEASIBILITY authority honored (GAP-007 confidence; no persistence of confidence status — read-time projection; no schema; PSOM-08 suppression contract consumed). Scope = `PopulationMapPointDto` (additive `IsLowConfidence`) + `EntityPopulationMapEngine` (read-time flag) + TS `PopulationMapPoint` model + 2 focused tests + tracker; no expansion, no missing scope, no repository/producer/DTO-field changes. Build evidence: VS MSBuild 18 (Release) green; focused `EntityPopulationMapEngineTest` 8/8 green. INFO-001: the flag derives from the presence of both resolved preset axis KPI values (`true` when either is absent/suppressed), consistent with GAP-007 quadrant exclusion and PSOM-08's value-suppression representation. INFO-002: the TS field is optional/additive to avoid breaking existing specs; PSOM-14 consumes it for quadrant exclusion. | — |
| PSOM-11 | 4 | Signed Y-axis projection | 5/5 | IMPLEMENTED | 2026-09-13: Added `isPercentAxisUnit` and sign-preserving symmetric-log `businessToSignedLog` to `robustStats.ts`; `businessToProjectedLog` now routes `Percent` units through the signed transform while IDR/days/default still clamp negatives at 0. `RobustProjectionStrategy.validateEntities` no longer clamps negative percent-axis values to 0, so negative YoY MTD Growth projects below business zero with sign and magnitude ordering preserved; projection bounds (computed from the dataset min/max with padding) enclose business zero when data spans both signs. Revised `populationProjectionEngine.spec.ts`: renamed the non-signed negative-clamp assertion, added `businessToSignedLog` / `businessToProjectedLog` signed tests, and added a PSOM-11 signed-percent block (negative not clamped + ordering, bounds enclose zero, non-percent axes still clamp). `vitest run` 303/303 green; `vue-tsc -b` clean. | 2026-09-13: GO. AC1–AC4 PASS; FEASIBILITY authority honored (GAP-005: signed Y-axis, negative growth not clamped, auto min/max bounds; rendering/axis-only change, no data/schema/KPI-calc impact). Scope = `robustStats.ts` (+2 helpers + `businessToProjectedLog` Percent branch), `robustProjectionStrategy.ts` (conditional clamp in `validateEntities`), `populationProjectionEngine.spec.ts` (revised + 8 signed tests), tracker; no expansion, no missing scope. Build evidence: `vitest run` 303/303 green; `vue-tsc -b` clean. INFO-001: signed transform is a symmetric-log (symlog) — the minimal signed extension of the existing log+robust-normalize pipeline, preserving monotonic sign/ordering as required; a linear projection would be a larger rewrite (LinearProjectionStrategy is still a placeholder). INFO-002: `computeProjectionBounds` unchanged — it already encloses business zero when data spans both signs because the signed transform maps 0→0 (verified by the new bounds test); the Y=0 reference-line rendering is deferred to PSOM-12 per plan. | — |
| PSOM-12 | 4 | Y=0 reference line and auto bounds | 2/5 | GO | 2026-09-13: Added `resolveBusinessZeroProjection` / `isBusinessZeroWithinBounds` / `ensureZeroTick` to `populationMapLayout.ts` (business 0 projected with the same log + robust-norm transform as plotted points; zero tick inserted sorted, no-op outside bounds or when already present). `PopulationMapCanvas.vue` draws a distinct horizontal business-zero line (`WORKSPACE_MAP_QUADRANT_LINE`, 1.25px solid; regression stays `WORKSPACE_REFERENCE_LINE` 1.5px diagonal) via `drawBusinessZeroLine` after `drawRegressionLine`, clipped to the plot and rendered only when 0 is within the auto-calculated bounds; `drawAxes` guarantees the `0` tick/label at the same position. Bounds computation unchanged (PSOM-11 auto bounds already enclose 0 when data spans both signs). Added 3 focused tests to `populationMapLayout.spec.ts` (zero projection within bounds, tick insertion + ordering, outside-bounds/already-present no-ops). `vitest run` 306/306 green; `vue-tsc -b` clean. | 2026-09-13: GO. AC1–AC3 PASS; FEASIBILITY authority honored (GAP-005 rendering/axis-only; no data/schema/KPI-calc/SA04 change; bounds reuse PSOM-11 auto bounds). Scope = `populationMapLayout.ts` (3 pure helpers) + `PopulationMapCanvas.vue` (zero line + tick wiring + draw order) + `populationMapLayout.spec.ts` (3 focused tests) + tracker; no expansion, no missing scope. Build evidence: `vitest run` 306/306 green; `vue-tsc -b` clean. | — |
| PSOM-13 | 4 | Fixed business quadrants | 4/5 | GO | 2026-09-13: Added fixed business quadrants for the `principal-sales-out-map` preset (GAP-006). `populationMapLayout.ts`: exported `PRINCIPAL_SALES_OUT_MAP_PRESET_ID`, `PACING_QUADRANT_X_THRESHOLD` (100) / `PACING_QUADRANT_Y_THRESHOLD` (0), `PacingQuadrant` + `PACING_QUADRANT_LABELS`, pure `classifyPacingQuadrant` (Star/Growing/Steady/Declining; `null` on missing/non-finite value), generalized `resolveBusinessProjection` (`resolveBusinessZeroProjection` now delegates; PSOM-12 behavior preserved), and `resolvePacingQuadrantBoundaries` converting business X=100% / Y=0% to screen coordinates via the same transform as plotted points (`null` outside visible bounds). `PopulationMapCanvas.vue`: for the fixed preset, draw the vertical X=100% boundary (`WORKSPACE_MAP_QUADRANT_LINE`) plus four corner labels (Star/Growing/Steady/Declining) and skip the statistical `Expected`/`Above Expected`/`Below Expected` region labels; other presets unchanged. Added 5 focused tests to `populationMapLayout.spec.ts` (four-quadrant classification, exact-threshold semantics incl. Star at 100/0, null handling, boundary alignment with the plotted transform, out-of-bounds null). `vitest run` 311/311 green; `vue-tsc -b` + `vite build` green. No backend/schema/PresetId change. | 2026-09-13: GO. AC1–AC4 PASS; FEASIBILITY authority honored (GAP-006 fixed business quadrants at X=100%/Y=0% — Star/Growing/Steady/Declining — replacing statistical regions; OQ-002 thresholds fixed, not configurable; statistical/population-relative boundaries out of scope; rendering-only, no data/schema/KPI-calc/SA04 change). Scope = `populationMapLayout.ts` (preset id + fixed thresholds + pure classifier + boundary-to-screen projection; PSOM-12 `resolveBusinessZeroProjection` refactored to delegate, behavior preserved), `PopulationMapCanvas.vue` (conditional vertical X=100% boundary + four corner labels + statistical-label skip for the preset), `populationMapLayout.spec.ts` (5 focused tests), tracker; no expansion, no missing scope, other presets unaffected. Build evidence: `vitest run` 311/311 green; `vue-tsc -b` clean; `vite build` green. INFO-001: the four labels are placed at plot corners (correct quadrant orientation) rather than offset from the boundary intersection; acceptable because the preset's boundaries fall within the data range and quadrant-boundary geometry is covered by the pure service tests. INFO-002: `classifyPacingQuadrant` is a pure service consumed by tests now and by PSOM-14 for low-confidence quadrant exclusion; no residual-based classification is used for the fixed preset. INFO-003: the horizontal Y=0 boundary remains owned by PSOM-12 `drawBusinessZeroLine`; PSOM-13 adds only the vertical X=100% line to avoid a duplicate draw. | — |
| PSOM-14 | 4 | Low-confidence treatment/exclusion | 3/5 | IMPLEMENTED | 2026-09-13: Added PSOM-14 low-confidence quadrant exclusion/styling (GAP-007). `populationMapLayout.ts`: `isLowConfidencePoint(point)` (flag helper) and `resolvePacingQuadrantForPoint(point)` returning `null` when `IsLowConfidence` is true (no business quadrant) and otherwise delegating to the PSOM-13 fixed-threshold classifier. `PopulationMapCanvas.vue`: low-confidence points are skipped by the statistical tier passes and drawn as distinct hollow markers (`LOW_CONFIDENCE_POINT_COLOR/OPACITY/RADIUS/STROKE_WIDTH`) via a dedicated `drawLowConfidencePoints` pass; a bottom-left `iw-map-confidence-note` shows the count of low-confidence entities excluded from quadrants for the `principal-sales-out-map` preset. `PopulationMapTooltip.vue`: surfaces a "Low confidence — excluded from quadrants" section. Added `.iw-map-confidence-note` styling to `investigation-workspace-tokens.css`. Added 4 focused tests to `populationMapLayout.spec.ts` (flagged exclusion across all four quadrants, non-flagged normal classification, absent-flag default, state helper). `npm test` 315/315 green; `npm run build` (`vue-tsc -b` + `vite build`) green. No backend/schema/DTO change; existing presets unaffected. | 2026-09-13: GO. AC1–AC3 PASS; FEASIBILITY authority honored (GAP-007: low-confidence entities assigned no business quadrant, visually distinguished; fixed thresholds from PSOM-13 unchanged; rendering-only, no data/schema/KPI-calc/SA04 change). Scope = `populationMapLayout.ts` (+`isLowConfidencePoint`/`resolvePacingQuadrantForPoint`), `PopulationMapCanvas.vue` (skip flagged in tier passes + hollow-marker pass + quadrant-exclusion note), `PopulationMapTooltip.vue` (confidence label), `investigation-workspace-tokens.css` (note style), `populationMapLayout.spec.ts` (4 focused tests); no expansion, no missing scope, no backend change. Build evidence: `npm test` 315/315 green; `npm run build` (`vue-tsc -b` + `vite build`) green. INFO-001: per the PSOM-10 contract low-confidence entities carry a suppressed axis and are still filtered from the scatter by `visiblePoints`; the hollow-marker pass applies to any flagged point that retains both axes, while the `iw-map-confidence-note` makes the exclusion user-visible today. INFO-002: `resolvePacingQuadrantForPoint` is a pure contract (canvas consumes `isLowConfidencePoint`), consistent with the PSOM-13 `classifyPacingQuadrant` precedent. | — |
| PSOM-15 | 5 | Backend verification suite | 3/5 | IMPLEMENTED | 2026-09-13: Completed the deferred PSOM-04 range-evidence reconciliation with two SQL-level reconciliation tests added to `PrincipalSalesOutRangeEvidenceDalTest`: a full-month `[MonthStart, MonthEnd]` range shares the exact source (`BTR_Faktur → BTR_FakturItem → BTR_Brg → BTR_Supplier`), void/blank filters, `SUM(ISNULL(fi.SubTotal,0) − ISNULL(fi.DiscRp,0))` expression, and per-Principal grouping with `PrincipalSalesOutHistoryEvidenceDal`, differing from the month-grain history only by the inclusive `BETWEEN @StartDate AND @EndDate` window predicate and the intentionally dropped `YEAR/MONTH` grouping — so summing a full month via the range query reconciles to `BTRPD_PrincipalSalesOutHistory`. Confirmed every PSOM test class is registered in `btr.test.csproj` (no new file needed; same file extended). Backend suite per PSOM-15 AC (`PrincipalKpiCatalogTest`, `AnalyticsPeriodCalculatorTest`, range-evidence reconciliation, `PrincipalAchievementComposerTest`, `PrincipalPacingAchievementComposerTest`, `PrincipalYoyGrowthComposerTest`, `PrincipalYoyMtdGrowthComposerTest`, `PrincipalConfidenceGuardTest`, `SupplierEntityAnalyticsProducerTest`, `EntityMapPresetRegistryTest`, `EntityInvestigationLensRegistryTest`, `EntityAnalyticsKpiRegistryTest`, `EntityPopulationMapEngineTest`). VS 2022 MSBuild (Release) build green; focused `dotnet vstest` 110/112 — 2 known pre-existing baseline failures (`EntityAnalyticsKpiRegistryTest.CustomerDefaultPack_ContainsCatalogBackedKpiIds`, `SupplierEntityAnalyticsProducerTest.Produce_WritesL4RelationshipRows`), no new failures; `PrincipalSalesOutRangeEvidenceDalTest` 5/5 green. | 2026-09-13: GO. AC1–AC3 PASS; FEASIBILITY authority honored (verification-only slice tracing to all closed gaps; no new business/architecture/feasibility decisions; no production/source/schema change). Scope = `PrincipalSalesOutRangeEvidenceDalTest.cs` (2 SQL-level reconciliation tests + shared-fragment/normalize helpers, +51 lines) + tracker; no expansion, no missing scope. AC1 all listed test classes present and registered in `btr.test.csproj` (no unregistered file); reconciliation closed PSOM-04 INFO-001. AC2 VS 2022 MSBuild (Release) build green; `dotnet vstest` focused suite 110/112; new range-evidence class 5/5. AC3 no new failures — the 2 failures (`EntityAnalyticsKpiRegistryTest.CustomerDefaultPack_ContainsCatalogBackedKpiIds`, `SupplierEntityAnalyticsProducerTest.Produce_WritesL4RelationshipRows`) are the pre-existing baseline documented at PSOM-03/PSOM-07/PSOM-08. INFO-001: the AC's literal `dotnet msbuild` command remains unusable for `btr.test` (dotnet SDK lacks VS web targets for the transitive `btr.portal.api`); VS 2022 MSBuild used per established repo convention. INFO-002: DB-backed reconciliation execution is unavailable (`Database.ServerName = "SERVER"`); reconciliation is verified at SQL-shape level (shared source/joins/filters/aggregation; only inclusive window + dropped YEAR/MONTH grouping differ), consistent with the PSOM-04 deferral. | — |
| PSOM-16 | 5 | Frontend verification suite | 3/5 | GO | 2026-09-13: Added `principalSalesOutMapVerification.spec.ts` — an integrated frontend verification suite for the time-aware Principal Sales-Out Map preset: (1) signed Y-axis projection retains negative YoY MTD Growth and business-Y ordering, and auto-computed bounds enclose business zero across signs; (2) non-percent (IDR) axes retain non-negative clamping (other-preset regression guard); (3) the `Y=0` reference projects to the plotted zero and `ensureZeroTick` inserts a `0` tick at the same screen position (and is a no-op outside bounds); (4) fixed business quadrant classification (Star/Growing/Steady/Declining) plus quadrant boundaries aligned to the plotted `(100, 0)` point and `PACING_QUADRANT_LABELS`; (5) low-confidence points are excluded from quadrants while unflagged points classify normally. Existing PSOM-11/12/13/14 focused specs retained unchanged (`populationProjectionEngine.spec.ts` signed behavior; `populationMapLayout.spec.ts` signed bounds, `0` tick, quadrant boundaries). No source/schema/backend change. `npm test` 325/325 green (39 files; +10 new tests); `npm run build` (`vue-tsc -b && vite build`) green. | 2026-09-13: GO. AC1–AC3 PASS; FEASIBILITY authority honored (GAP-005 signed Y-axis/auto bounds/`Y=0` reference, GAP-006 fixed quadrants at X=100%/Y=0%, GAP-007 low-confidence exclusion; verification-only, no production/source/schema/backend change). Scope = 1 new frontend spec file (`principalSalesOutMapVerification.spec.ts`, 10 tests) + tracker; no expansion, no missing scope. Build evidence: `npm test` 325/325 green (39 files; +10 tests); `npm run build` (`vue-tsc -b && vite build`) green. INFO-001: existing PSOM-11..14 focused specs already cover the AC-named files (`populationProjectionEngine.spec.ts` signed behavior; `populationMapLayout.spec.ts` signed bounds, `0` tick, quadrant boundaries); the new suite adds integrated service-level verification across projection → bounds → zero → quadrant/confidence without altering prior-slice tests. | — |
| PSOM-17 | 5 | Knowledge and governance sync | 1/5 | IMPLEMENTED | 2026-09-13: Docs-only governance sync (GAP-009). `PRINCIPAL-KPI-REGISTRY.md`: added `PRN-TGT-004` (`Pacing Achievement %`, Percent, X-axis) and `PRN-GRW-003` (`YoY MTD Growth %`, Percent, Y-axis) rows with registered display names/units referencing `PrincipalKpiCatalog`/`SupplierEntityAnalyticsRegistrar` (no independent formulas); added Time-Aware Map KPIs + Governance (GAP-009) section stating registration before map/lens/profile/export consumption; `PRN-TGT-003`/`PRN-GRW-002` noted unchanged. `btr-portal-kpi-catalog.md`: registry range extended to `PRN-TGT-001–004`/`PRN-GRW-001–003` (17 entries), ToC + two full entries (`PRN-TGT-004`, `PRN-GRW-003`) with registered names/units and registry references, §6.9 governance paragraph (registered-before-consumption), unchanged notes on `PRN-TGT-003`/`PRN-GRW-002`; SA04 index row untouched. No code/schema/SA04 change. | 2026-09-13: GO. AC1–AC3 PASS; FEASIBILITY authority honored (GAP-009 registry-authoritative, registered-before-consumption; Option B additive; existing TGT-003/GRW-002 definitions + SA04 index + supporting list untouched; no persistence/schema/SA04). Scope = 2 docs + tracker; no expansion, no code. Build/tests N/A (docs-only slice, no test AC). INFO-001: pre-review self-check caught and fixed a `PRN-TGT-003` Unit label (`Ratio` per registrar, not `Percent`); new-KPI units (`Percent`) verified against registrar. | — |
