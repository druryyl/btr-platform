...
Deliver **Collection Dashboard** at `/dashboard/collection` (sidebar label: **Collection**) using **Attention-First layout with Recovery Summary**. Materialize KPIs in a **dedicated `BTR_PortalDashboardCollection*` snapshot domain** (Piutang open balance + Pelunasan sources; refresh cadence **30 minutes**). Mandatory sections (in order):
...
| Materialized snapshot worker | `RefreshDashboard*SnapshotWorker` | **Pattern** for dedicated `BTR_PortalDashboardCollection*` domain |
...
| `BTR_PortalDashboardPiutangTopCustomer` | Top 10 by balance |
| `BTR_PortalDashboardCustomer*` | Customer attention, top piutang |
| `BTR_PortalDashboardSalesman*` | Salesman overdue/piutang exposure |

**No `BTR_PortalDashboardCollection*` tables exist.**
...
| # | Decision |
| - | -------- |
| Q21 | **Dedicated snapshot domain:** `BTR_PortalDashboardCollection*` |
...
- [x] DSO excluded
- [x] Dedicated `BTR_PortalDashboardCollection*` snapshot; 30-minute refresh
