...
| S5.5 | 2026-09-16 | GO | None blocking. **INFO-001:** no automated test project targets `BGud` (same as S5.1 INFO-002 / S5.2 INFO-002 / S5.3 INFO-002 / S5.4 INFO-001); verification was by `:app:compileDebugKotlin --rerun-tasks` → `BUILD SUCCESSFUL` plus `:app:assembleDebug` → `BUILD SUCCESSFUL`. **INFO-002:** quick-action / navigation targets (`scan`, `barcode_registry`, `register`, `synchronization`, `settings`) resolve to §13.2 routes whose destinations are owned by S5.6..S5.11 and do not exist yet; wiring the documented route strings (rather than no-ops) is the faithful §13.2 navigation wiring for this slice. **INFO-003:** `officeCode` (login-returned `serverId`) is shown display-only in the context header, consistent with the S5.2/S5.4 precedent (ADR-007 §8: display + legacy I-06 read route only) — no request carries `ServerId`, and neither DataStore nor Room gains a tenant field. | None required. Scope: 3 new files (`viewmodel/HomeViewModel.kt`, `viewmodel/HomeViewModelFactory.kt`, `ui/screen/HomeScreen.kt`) + `ui/Navigation.kt` home-route wiring (placeholder removed as a direct consequence within the touched route) + tracker; scan/register/registry/edit/synchronization/settings screens (S5.6..S5.11) untouched. |
| S5.6 | 2026-09-16 | GO | None blocking. **INFO-001:** no automated test project targets `BGud` (same as S5.1 INFO-002 / S5.2 INFO-002 / S5.3 INFO-002 / S5.4 INFO-001 / S5.5 INFO-001); verification was by `:app:compileDebugKotlin --rerun-tasks` → `BUILD SUCCESSFUL` (zero warnings) plus `:app:assembleDebug` → `BUILD SUCCESSFUL` (one intermediate compile error — ML Kit `Barcode` import used the post-17.0 `…barcode.common` package while the declared `barcode-scanning` is 16.2.0, where the class lives in `…vision.barcode`; fixed before the passing build). **INFO-002:** §13.2 diagrams `not found + Cancel ───▶ back` while §14.2 transitions `Cancel ───▶ Scanning`; both are satisfied without a new decision — `Cancel` re-arms to `Scanning` (§14.2 literal) and a screen-level `Back` plus Found `Close` both pop the back stack (§13.2 literal). **INFO-003:** `LocalLifecycleOwner` is imported from `androidx.lifecycle.compose` (current API) rather than the deprecated `platform` accessor; no new dependency (`lifecycle-runtime-compose` already declared, S5.1). | None required. Scope: 4 new files (`viewmodel/ScanViewModel.kt`, `viewmodel/ScanViewModelFactory.kt`, `ui/component/BarcodeScannerView.kt`, `ui/screen/ScanScreen.kt`) + `ui/Navigation.kt` scan-route wiring (`scan` composable + `register?barcode={value}` route string; the register destination itself is owned by S5.7 and untouched) + tracker; no existing behavior modified; no new dependency; no `ServerId` stored or sent. |
| S5.7 | 2026-09-16 | GO | None blocking. **INFO-001:** no automated test project targets `BGud` (same as S5.1 INFO-002 / S5.2 INFO-002 / S5.3 INFO-002 / S5.4 INFO-001 / S5.5 INFO-001 / S5.6 INFO-001); verification was by `:app:compileDebugKotlin --rerun-tasks` → `BUILD SUCCESSFUL` plus `:app:assembleDebug` → `BUILD SUCCESSFUL` (first attempt, zero errors). **INFO-002:** `BarangDao.search` (LIKE over `brgCode`/`brgName`, `LIMIT 50`) is a supporting prerequisite not named in the §3 impact inventory, required for the §12.6 local Item search; user-supplied `%`/`_` act as LIKE wildcards (unescaped) — over-match only, no guardrail risk. **INFO-003:** home → `register` with no barcode argument yields a blank read-only barcode and permanently disabled `Save` (only Cancel/back is useful); the plan mandates a read-only captured barcode with no manual-entry path on this screen, so no entry path was invented — flagged for planner awareness. | None required. Scope: 3 new files (`viewmodel/RegisterBarcodeViewModel.kt`, `viewmodel/RegisterBarcodeViewModelFactory.kt`, `ui/screen/RegisterScreen.kt`) + `BarangDao.search` prerequisite + `ui/Navigation.kt` `register?barcode={barcode}` route (nullable arg; `register` bare route serves the home quick action) + tracker; scan/home/login behavior untouched; no new dependency; no network call on the slice path; no `ServerId` stored or sent. |
| S5.8 | 2026-09-16 | GO | None blocking. **INFO-001:** no automated test project targets `BGud` (same as S5.1 INFO-002 / S5.2 INFO-002 / S5.3 INFO-002 / S5.4 INFO-001 / S5.5 INFO-001 / S5.6 INFO-001 / S5.7 INFO-001); verification was by `:app:compileDebugKotlin --rerun-tasks` → `BUILD SUCCESSFUL` plus `:app:assembleDebug` → `BUILD SUCCESSFUL` (first attempt, zero errors). **INFO-002:** `BarcodeDao.search` (LIKE over `barcodeValue`/`brgCode`/`brgName`, paged `LIMIT`/`OFFSET`) is a supporting prerequisite not named in the §3 impact inventory, required for the §12.7 searchable list; user-supplied `%`/`_` act as LIKE wildcards (unescaped) — over-match only, no guardrail risk, mirroring S5.7 INFO-002. **INFO-003:** the `edit?barcodeId={id}` row-action target (§13.2) is wired as a documented route string only; the edit destination itself is owned by S5.9 and untouched — same forward-reference pattern as S5.5 INFO-002 / S5.6 scope. | None required. Scope: 3 new files (`viewmodel/BarcodeRegistryViewModel.kt`, `viewmodel/BarcodeRegistryViewModelFactory.kt`, `ui/screen/BarcodeRegistryScreen.kt`) + `BarcodeDao.search` prerequisite + `ui/Navigation.kt` `barcode_registry` composable + tracker; login/home/scan/register behavior untouched; no new dependency; no network call on the slice path; no `ServerId` stored or sent. |
| S5.9 | 2026-09-16 | GO | None blocking. **INFO-001:** no automated test project targets `BGud` (same as S5.1 INFO-002 / S5.2 INFO-002 / S5.3 INFO-002 / S5.4 INFO-001 / S5.5 INFO-001 / S5.6 INFO-001 / S5.7 INFO-001 / S5.8 INFO-001); verification was by `:app:compileDebugKotlin --rerun-tasks` → `BUILD SUCCESSFUL` plus `:app:assembleDebug` → `BUILD SUCCESSFUL` (first attempt, zero errors). **INFO-002:** `BarcodeDao.getById` (lookup by `brgBarcodeId`) is a supporting prerequisite not named in the §3 impact inventory, required to load the §13.2 `edit?barcodeId={id}` correction target — same prerequisite pattern as S5.7 INFO-002 / S5.8 INFO-002. **INFO-003:** the `Mapping Saat Ini` card is a display-only readout of the loaded cached row (correction-target context for §14.4 `Loaded`); it edits nothing and introduces no §12.8 layout deviation — Barcode read-only, Item Search, Unit optional, Save/Cancel only. **INFO-004:** no local duplicate probe on this path (the barcode identity itself is the cached correction target); uniqueness remains Main Office–authoritative (§18.1), consistent with §18.4 best-effort scoping. | None required. Scope: 3 new files (`viewmodel/EditBarcodeViewModel.kt`, `viewmodel/EditBarcodeViewModelFactory.kt`, `ui/screen/EditBarcodeScreen.kt`) + `BarcodeDao.getById` prerequisite + `ui/Navigation.kt` `edit?barcodeId={barcodeId}` composable (closes the S5.8 INFO-003 forward reference) + tracker; login/home/scan/register/registry behavior untouched; no new dependency; no network call on the slice path; no `ServerId` stored or sent; no activation/deactivation surface (IR-06, C-1). |
| S5.10 | 2026-09-16 | GO | None blocking. **INFO-001:** no automated test project targets `BGud` (same as S5.1 INFO-002 / S5.2 INFO-002 / S5.3 INFO-002 / S5.4 INFO-001 / S5.5 INFO-001 / S5.6 INFO-001 / S5.7 INFO-001 / S5.8 INFO-001 / S5.9 INFO-001); verification was by `:app:compileDebugKotlin --rerun-tasks` → `BUILD SUCCESSFUL` (first attempt, zero errors) plus `:app:assembleDebug` → `BUILD SUCCESSFUL`. **INFO-002:** `syncedCount()` / `rejectedCount()` on `BarcodeRegistrationRequestDao` are supporting prerequisites not named in the §3 impact inventory, required for the §12.9 Queue Card (Success/Rejected) — same prerequisite pattern as S5.7 INFO-002 / S5.8 INFO-002 / S5.9 INFO-002, mirroring the existing `pendingCount()`. **INFO-003:** `SyncState` (`IDLE`/`SYNCHRONIZING`/`SYNCHRONIZED`/`FAILED`) extends the §19.3 key fields (`isSyncing`, `error` retained verbatim) as the direct realization of the §14.5 state machine; blank `baseUrl` (C-3) maps to `FAILED` with the S5.4 error-region message instead of issuing a call. | None required. Scope: 3 new files (`viewmodel/SynchronizationViewModel.kt`, `viewmodel/SynchronizationViewModelFactory.kt`, `ui/screen/SynchronizationScreen.kt`) + DAO prerequisite + `ui/Navigation.kt` `synchronization` composable (closes the S5.5 INFO-002 forward reference for this route; `settings` remains owned by S5.11) + tracker; login/home/scan/register/registry/edit behavior untouched; no new dependency; no network call from the screen path (worker S5.3 issues I-04/I-05/I-06/I-09); no `ServerId` stored or sent (`officeCode` passed as worker input for the legacy I-06 read route only, S5.3/S5.4 precedent); IR-M5 (offline disables Sync Now) and IR-M6 (one run at a time; Kembali stays enabled as Cancel/Close) satisfied. |
| S5.11 | 2026-09-16 | GO | None blocking. **INFO-001:** no automated test project targets `BGud` (same as S5.1 INFO-002 / S5.2 INFO-002 / S5.3 INFO-002 / S5.4 INFO-001 / S5.5 INFO-001 / S5.6 INFO-001 / S5.7 INFO-001 / S5.8 INFO-001 / S5.9 INFO-001 / S5.10 INFO-001); verification was by `:app:compileDebugKotlin --rerun-tasks` → `BUILD SUCCESSFUL` (first attempt, zero errors) plus `:app:assembleDebug` → `BUILD SUCCESSFUL`. **INFO-002:** UX Blueprint §4 defines `Settings` only as a navigation leaf with no field specification, and Architecture SCR-MOB-008 defers to it; the surface (Session Card display-only + Logout + Kembali) realizes the leaf with no invented business capability — session readout mirrors the Home context header (S5.5 precedent), `officeCode` display-only (ADR-007 §8), no Room clearing (cache replacement stays with the login sync, S5.3/S5.4). **INFO-003:** `onLoggedOut` navigates to `login` with `popUpTo("home") { inclusive = true }`; the DataStore session gate re-reads the cleared token so the start-destination logic (`login` when no session, else `home`) holds without a new decision — warehouse change is logout + re-authentication (IR-09). | None required. Scope: 3 new files (`viewmodel/SettingsViewModel.kt`, `viewmodel/SettingsViewModelFactory.kt`, `ui/screen/SettingsScreen.kt`) + `ui/Navigation.kt` `settings` composable (closes the S5.5 INFO-002 forward reference for this route; all §13.2 destinations now exist) + tracker; login/home/scan/register/registry/edit/synchronization behavior untouched; no new dependency; no network call on the slice path; no `ServerId` stored or sent; no activation/deactivation surface (IR-06, C-1). |
